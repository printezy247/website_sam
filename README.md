...
| 🧠 LLM | Add any of `GEMINI_API_KEY`, `GROQ_API_KEY`, `NVIDIA_API_KEY`, `OPENROUTER_API_KEY`, `MOONSHOT_API_KEY`, `OLLAMA_API_KEY`, `LLM_BASE_URL`+`LLM_API_KEY`, `ANTHROPIC_API_KEY`. All keys are used in this order: gemini, ollama, openrouter, nvidia, moonshot, groq, custom, anthropic. `LLM_PROVIDER` moves one to the front; the rest are failover. Models are discovered per key; `/admin/articles` lists them and shows which one each provider will use. Pin per provider with `LLM_MODEL_NVIDIA`, `LLM_MODEL_MOONSHOT`, `LLM_MODEL_OLLAMA`, `LLM_MODEL_OPENROUTER`, `LLM_MODEL_GEMINI` (exact id, or words such as `nemotron 3.5 lightning` matched against the live list). Test one with `/api/cron/article?key=…&wait=1&provider=nvidia&model=meta/llama-3.3-70b-instruct` |
