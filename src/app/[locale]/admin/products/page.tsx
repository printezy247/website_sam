...
function Form({ p }: { p?: typeof products.$inferSelect }) {
  return (
    <form action={adminUpsertProduct} className="glass rounded-xl p-4 grid gap-2 md:grid-cols-4 text-sm">
      ...
      <select name="tierIncluded" defaultValue={p?.tierIncluded ?? ""} className={inp}><option value="">not included in any tier</option><option value="free">free</option><option value="pro">pro</option><option value="elite">elite</option></select>
