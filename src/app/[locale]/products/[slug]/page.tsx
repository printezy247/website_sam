...
export default async function Product({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-16">
      ...
      <StickyBuyBar name={p.name} price={p.priceCents ? `$${p.priceCents / 100}` : "Free"} note={p.tierIncluded ? t("sticky_included", { tier: tt(p.tierIncluded as "free") }) : undefined} href={`/account?product=${p.slug}`} label={t("buy")} />
    </div>
  );
}
